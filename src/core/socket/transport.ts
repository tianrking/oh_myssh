 ...
 * 隔离 Chromium Direct Sockets (IWA)、WebSocket Relay 以及开发环境 Mock Socket
 */
...
/**
 * Mock Interactive Shell Transport (用于纯前端开发调试、演示与离线模拟)
 */
export class MockSocketTransport implements SocketTransport {
  ...
  async connect(host: string, port: number): Promise<DuplexByteStream> {
    ...
    const prompt = () => {
      if (controllerRef) {
        controllerRef.enqueue(encoder.encode(`\x1b[32mroot@${host}\x1b[0m:\x1b[34m~\x1b[0m# `));
      }
    };
    ...
    const readable = new ReadableStream<Uint8Array>({
      start(ctrl) {
        ...
        setTimeout(() => {
          ...
          sendLine(` System load: 0.12, Memory usage: 24%, Local Time: ${new Date().toLocaleString()}`);
          sendLine(`\x1b[1;36m====================================================\x1b[0m`);
          sendLine(``);
          prompt();
        }, 150);
      ...
      write(chunk) {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          if (char === '\r' || char === '\n') {
            ...
            currentInput = '';
            if (cmd === 'help') {
              sendLine('\x1b[33mOh My SSH Built-in Mock Terminal\x1b[0m');
              sendLine('Available commands: help, uname, ls, top, clear, exit, whoami, neofetch');
              ...
              sendLine('drwxr-xr-x 4 root root 4096 Jul 24 10:48 \x1b[34mprojects\x1b[0m');
              sendLine('drwxr-xr-x 2 root root 4096 Jul 24 10:48 \x1b[34mscripts\x1b[0m');
              sendLine('-rw-r--r-- 1 root root 1024 Jul 24 10:48 \x1b[32mdeploy.sh\x1b[0m');
              sendLine('-rw-r--r-- 1 root root 2048 Jul 24 10:48 docker-compose.yml');
            } else if (cmd === 'neofetch') {
              sendLine('  \x1b[31m/\\_/\\\x1b[0m   \x1b[1;33mroot@' + host + '\x1b[0m');
              sendLine(' \x1b[31m( o.o )\x1b[0m  --------------');
              sendLine('  \x1b[31m> ^ <\x1b[0m   OS: Oh My SSH WebAssembly OS');
            ...
            if (currentInput.length > 0) {
              ...
              if (controllerRef) {
                controllerRef.enqueue(encoder.encode('\b \b'));
              }
            }
          } else {
            currentInput += char;
            if (controllerRef) {
              controllerRef.enqueue(chunk.subarray(i, i + 1));
            }
          }
        }
      },
