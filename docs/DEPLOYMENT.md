...
2. 输入密码，或粘贴受支持的私钥。
3. 首次收到 host-key 时浏览器自动保存 SHA-256 指纹，不弹原生确认框。
4. 后续连接会静默校验指纹；指纹变化会阻断，不能自动覆盖旧记录。

高风险环境仍建议在第一次连接前通过云厂商控制台或服务器可信渠道核对指纹；
无原生弹窗是纯网页产品的可用性取舍，不代表 Worker 会跳过 host-key 校验。
